...
import { listGames, type GameRow } from "@/lib/games.functions";
import { STICKER_LABELS, STICKER_STYLES, gameImageSrc } from "@/lib/game-display";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "GamePlay | Аренда игровых консолей в Нальчике" },
      {
        ...
        content: "Посуточная аренда игровых приставок PlayStation 5 и PlayStation 4 в Нальчике.",
      },
...
const consoles = [
  {
    name: "PS4 SLIM",
    tag: "Classic",
    desc: "Идеально для вечерних посиделок",
    price: "700",
    img: ps4Img,
  },
...
const steps = [
  { n: "01", title: "Выбор", text: "Выбираете консоль на нашем сайте.", icon: Monitor },
  {
    n: "02",
    title: "Заявка",
    text: "Позвоните или напишите нам в WhatsApp/Telegram для брони.",
    icon: Phone,
  },
  {
    n: "03",
    title: "Получение",
    text: "Забираете комплект (потребуется паспорт для оформления договора).",
    icon: PackageCheck,
  },
  {
    n: "04",
    title: "Игра",
    text: "Подключаете к ТВ и наслаждаетесь классной игровой атмосферой!",
    icon: Gamepad2,
  },
];

const AVITO_URL =
  "https://www.avito.ru/nalchik/predlozheniya_uslug/arenda_ps5ps4_prokat_pleysteshen_5_3639072809";

const reviews = [
  {
    name: "Тембулат Петров",
    date: "12 февраля",
    text: "Все отлично. Сразу ответили, проинформировали. Приехал, забрал, всю ночь играл — никаких проблем не было.",
  },
  {
    name: "ИП Лавров",
    date: "20 мая",
    text: "Очень удобный сервис! Приставка с двумя контроллерами и всеми проводами, удобно упакована в рюкзак для перевозки. Большой выбор игр. Однозначно буду пользоваться!",
  },
  {
    name: "Archer",
    date: "23 января",
    text: "Хороший парень, все чётко, добровестный прокатчик… 👌🤝",
  },
];
...
const faqs = [
  {
    ...
    a: "В стандартный комплект входит: игровая приставка, 2 беспроводных геймпада (DualSense для PS5 или DualShock 4 для PS4), а также все необходимые провода: кабель питания, HDMI-кабель и 2 шнура для зарядки джойстиков.",
  },
  {
    q: "Где мы находимся?",
    a: "Находимся в г. Нальчик на улице Осетинская 8 (торговый ряд «КОЛОНКА»).",
  },
  {
    q: "Во сколько можно забрать приставку?",
    a: "Работаем с 9:00 до 23:00, приезжайте в удобное для Вас время.",
  },
  {
    q: "А что если я случайно сломаю геймпад?",
    a: "Согласно договору аренды, арендатор несёт полную имущественную ответственность за порчу или поломку арендованного имущества. Мы просим относиться к технике бережно.",
  },
  {
    q: "Нужен ли залог или паспорт?",
    a: "Для оформления аренды заключается официальный договор. В качестве залога вы можете оставить либо паспорт, либо денежную сумму (размер залога уточняется при бронировании), на ваше усмотрение.",
  },
  {
    q: "До скольки можно играть?",
    a: "Минимальный срок аренды составляет 24 часа. Например, если вы взяли приставку в 14:00, её необходимо будет вернуть на следующий день в это же время (14:00).",
  },
];
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-md bg-background/70 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-muted-foreground">
            <a href="#consoles" className="hover:text-foreground transition">
              Приставки
            </a>
          ...
          <button
            ...
            className="md:hidden text-foreground p-2 rounded-lg hover:bg-primary/10 active:scale-[0.88] active:brightness-125 transition-all duration-150"
            aria-label="Menu"
            aria-expanded={menuOpen}
          >
            <div className="relative w-6 h-6">
              <span
                className={`absolute left-0 top-1/2 block h-0.5 w-6 bg-current rounded transition-all duration-300 ease-out ${menuOpen ? "rotate-45 translate-y-0" : "-translate-y-2"}`}
              />
              <span
                className={`absolute left-0 top-1/2 block h-0.5 w-6 bg-current rounded transition-all duration-200 ease-out ${menuOpen ? "opacity-0 scale-x-0" : "opacity-100 scale-x-100"}`}
              />
              <span
                className={`absolute left-0 top-1/2 block h-0.5 w-6 bg-current rounded transition-all duration-300 ease-out ${menuOpen ? "-rotate-45 translate-y-0" : "translate-y-2"}`}
              />
          ...
          <div className="px-6 py-8 flex flex-col items-center gap-6 text-2xl font-display font-bold uppercase tracking-wider">
            ...
            <div className="flex items-center justify-center gap-4 pt-4 border-t border-border flex-wrap">
              <a
                ...
                aria-label="WhatsApp"
                className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-[#25D366] hover:border-[#25D366] hover:shadow-[0_0_20px_#25D36680] hover:scale-110 active:scale-90 active:brightness-125 transition-all duration-150"
              >
              ...
              <a
                ...
                rel="noopener noreferrer"
                aria-label="Telegram"
                className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-[#0088cc] hover:border-[#0088cc] hover:shadow-[0_0_20px_#0088cc80] hover:scale-110 active:scale-90 active:brightness-125 transition-all duration-150"
              >
              ...
              <a
                ...
                className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-[#E1306C] hover:border-[#E1306C] hover:shadow-[0_0_20px_#E1306C80] hover:scale-110 active:scale-90 active:brightness-125 transition-all duration-150"
              >
              ...
              <a
                ...
                rel="noopener noreferrer"
                aria-label="VK"
                className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-[#0077FF] hover:border-[#0077FF] hover:shadow-[0_0_20px_#0077FF80] hover:scale-110 active:scale-90 active:brightness-125 transition-all duration-150"
              >
      ...
      <section id="top" className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <a
              href="#consoles"
              className="rounded-full px-8 py-4 bg-primary text-primary-foreground font-display font-bold uppercase tracking-wider hover:brightness-110 hover:shadow-[var(--shadow-neon)] active:scale-[0.92] active:brightness-125 transition-all duration-150 shadow-[var(--shadow-neon)]"
            >
              Каталог приставок
            </a>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 mt-12">
            {consoles.map((c) => (
              <div
                key={c.name}
                className="group relative rounded-2xl bg-card border border-border overflow-hidden hover:border-primary transition"
              >
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    <span className="shrink-0 text-xs px-3 py-1 rounded-full bg-primary text-primary-foreground font-semibold uppercase">
                      {c.tag}
                    </span>
                  </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
            {steps.map((s) => {
              ...
              return (
                <div
                  key={s.n}
                  className="relative p-8 rounded-2xl bg-card border border-border hover:neon-border transition"
                >
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="flex gap-6 w-max animate-marquee group-hover:[animation-play-state:paused]">
              {[...reviews, ...reviews].map((r, idx) => (
                <div
                  key={idx}
                  className="p-6 rounded-2xl bg-card border border-border w-[320px] sm:w-[360px] shrink-0"
                >
          ...
          <div className="mt-10 flex justify-center">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-full px-8 py-4 bg-accent text-accent-foreground font-display font-bold uppercase tracking-wider hover:brightness-110 hover:shadow-[var(--shadow-cyan)] active:scale-[0.92] active:brightness-125 transition-all duration-150 shadow-[var(--shadow-cyan)]"
            >
            ...
            {faqs.map((f, i) => (
              <div key={i} className="rounded-2xl bg-card border border-border overflow-hidden">
                <button
                  ...
                  className="w-full p-6 flex items-center justify-between text-left font-display font-bold text-lg active:brightness-110 transition-all duration-150"
                >
                  <span>{f.q}</span>
                  <span
                    aria-hidden
                    className={`text-primary transition-transform ${openFaq === i ? "rotate-45" : ""}`}
                  >
                    +
                  </span>
                </button>
                {openFaq === i && (
                  <div id={`faq-answer-${i}`} className="px-6 pb-6 text-muted-foreground">
                    {f.a}
                  </div>
                )}
              </div>
      ...
      <footer id="footer" className="border-t border-border py-12 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8 text-center md:text-left items-start">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-2">
              Аренда консолей в Нальчике, ул.Осетинская 8
            </p>
          </div>
          <div>
            {/* Яндекс Карта (тёмная тема) */}
            <div className="mb-4 rounded-2xl overflow-hidden border border-border bg-background">
              <iframe
                ...
                style={{
                  filter:
                    "invert(0.92) hue-rotate(180deg) saturate(0.85) brightness(0.95) contrast(1.05)",
                }}
              />
            </div>
            <a
              href={PHONE_HREF}
              className="font-display text-xl font-bold hover:text-primary transition"
            >
              {PHONE}
            </a>
            <p className="text-muted-foreground text-sm mt-2">Работаем ежедневно с 9:00 до 23:00</p>
          </div>
          <div className="flex justify-center md:justify-end items-start gap-4 flex-wrap">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-[#E1306C] hover:border-[#E1306C] hover:shadow-[0_0_15px_#E1306C80] hover:scale-110 active:scale-90 active:brightness-125 transition-all duration-150"
              aria-label="Instagram"
            >
              <Instagram className="w-7 h-7" />
            </a>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-[#0077FF] hover:border-[#0077FF] hover:shadow-[0_0_15px_#0077FF80] hover:scale-110 active:scale-90 active:brightness-125 transition-all duration-150"
              aria-label="VK"
            >
              <VkIcon className="w-7 h-7" />
            </a>
        ...
        <div className="max-w-6xl mx-auto mt-10 pt-6 border-t border-border text-center text-xs text-muted-foreground uppercase tracking-widest flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4">
          <span>© {new Date().getFullYear()} GAMEPLAY NALCHIK. Все права защищены.</span>
          <Link
            to="/privacy-policy"
            className="hover:text-primary transition normal-case tracking-normal"
          >
...
function SectionTitle({
  ...
  return (
    <h2
      className={`font-display font-bold text-4xl sm:text-5xl uppercase ${align === "center" ? "text-center" : ""}`}
    >
...
function GameCard({
  ...
  game: GameRow;
  onOpen: (game: GameRow, el: HTMLElement) => void;
}) {
